...
<p>
Now suppose that the analyst is interested in knowing the true count under (Redfern, 70-79, Male) from Table T1 with access to only the TBE algorithm. The TBE essentially suppresses this small count and returns 0. The analyst can try something like this. Let <b>M</b> be the total number of males in Redfer (i.e., across all ages). The analyst can ask two queries: (10-19, 20-29,...,50-59) and (60-69, 70-79), receiving two tables from the TBE algorithm. Let <b>M1</b> the total number of males in the age range (10-59) and let <b>M2</b> denote the total number of males in the age range (60-79). Through the two tables, the analysts obtains noisy versions of these counts (through the total row). Let's call them <b>M1'</b> and <b>M2'</b>. Note that <b>M</b> = <b>M1</b> + <b>M2</b>. Notice also that <b>M1</b> and <b>M2</b> have different contributors (in fact, mutually exclusive). The analyst records the noisy sum, i.e., <b>M1'</b> + <b>M2'</b>. 
</p>

<p>
The analyst then chooses another "partition," e.g., (10-19, 20-29, ..., 40-49) and (50-59, 60-69, 70-79). Once again the sum of the number of males is equal to <b>M</b>. And the analyst receives noisy counts <b>M1''</b> and <b>M2''</b>. Now, since the partitions are different, the contributors are different as well, and hence these noisy counts are obtained by adding fresh noise! The analysts adds <b>M1''</b> and <b>M2''</b> to the noisy sum. Continuing on this way, the analyst obtains noisy counts for all "two-partitions". Each two-partition contains different contributors (unless the true count is 0; the analyst can discard queries whose answer under TBE is 0). For a total of m different attribute values, there are exactly $2^(m-1) - 1$ such two-partitions. For our case, we have m = 7, and so 63 different two-partitions. Adding the noisy total row counts for each query, the analyst can then divide the noisy sum by the total number of two-partitions queried, and obtain the actual count <b>M</b>. This averaging attack works the same way as the previous simpler averaging attack except now we are carefully asking queries to ensure that we get around the "same contributors, same noise" property of the TBE algorithm.
</p>

<p>
Once the analyst has obtained <b>M</b>, the analyst can do the same process to obtain the true count under <b>M</b> - (70-79) (by considering two-partitions without 70-79. Finally the true count of (Redfern, 70-79, Male) is obtained as the difference between the two answers. This is more or less the crux of the attack in our paper. I have excluded some technicalities (such as what to do when one of the two-partitions returns a zero count under TBE). These details are in our paper. For this post, notice that we can apply the same process to find the true count under each age range, and thus obtain the noiseless table for the age distribution of the entire Redfern+Male sub-population in the database. This table can then be converted into unit record format as explained before.
</p>
